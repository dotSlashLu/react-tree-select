...
class DslTreeSelect extends React.Component {
  ...
  componentWillMount() {
    ...
    // if mode is multi, but a string value is provided
  ...
  componentWillReceiveProps(nextProps) {
    // console.debug("received nextProps, value", nextProps.value,
    //   "\n == this.selected", nextProps.value == this.selected);
  ...
  nodeSelectHandler(value, isDeselect = false) {
    if (!this.props.multi) {
      if (isDeselect)
        ...
        this.selected = value;
      return this.props.changeHandler(this.selected)
    }
  ...
  */
  generateNode(ret, root, depth, idx, path = "") {
    ...
    ret.push(
      ...
      expandIconClassNames={this.props.expandIconClassNames}
      collapseIconClassNames={this.props.collapseIconClassNames}>
        {children}
      </Node>
    )
  ...
  render() {
    ...
    let wrapperCls = classNames(this.wrapperClassNames, "dsl-ts-wrapper"),
        treeCls = classNames(this.treeClassNames, "dsl-ts-tree");
    return (
